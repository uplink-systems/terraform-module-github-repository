...
resource "github_release" "release" {
  ...
  name                                    = each.value.name == null ? each.value.tag_name : each.value.name
  target_commitish                        = each.value.target_commitish == null ? "main" : each.value.target_commitish
  body                                    = each.value.body == null ? each.value.tag_name : each.value.body
  generate_release_notes                  = each.value.generate_release_notes == null ? true : each.value.generate_release_notes
  draft                                   = each.value.draft == null ? false : each.value.draft
  prerelease                              = each.value.prerelease == null ? false : each.value.prerelease
  discussion_category_name                = var.repository.has_discussions == false ? null : each.value.discussion_category_name
  depends_on                              = [ github_repository.repository, github_branch.branch ]
  lifecycle {
    ignore_changes  = [ body, generate_release_notes ]
  }
}